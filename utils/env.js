...
export class LinkedinConfig {
  ...
  static get JOB_FILTER_URL() {
    const encodedKeywords = encodeURIComponent(this.KEYWORDS.join(", "));
    const filter = `${this.JOBS_URL}/search/?keywords=${encodedKeywords}&f_E=${this.EXPERIENCE_LEVELS}&f_AL=${this.EASY_APPLY}&f_F=${this.FUNCTIONS}&f_JT=${this.JOB_TYPES}&f_TPR=${this.POSTED_WITHIN}&geoId=${this.LOCATION_ID}&origin=JOB_SEARCH_PAGE_SEARCH_BUTTON&refresh=true&sortBy=${this.SORT_BY}`;
    console.log(filter);
    return filter
